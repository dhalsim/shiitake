...
type ChannelView struct {
	*adaptive.LoadablePage
	...
	selectID string // delegate to be selected later
}
...
var lastOpenKey = app.NewSingleStateKey[string]("direct-last-open")

// NewChannelView creates a new view.
func NewChannelView(ctx context.Context) *ChannelView {
	v := ChannelView{
		ctx:      ctx,
		channels: make(map[string]*Channel, 50),
	}
	...
	v.list.ConnectRowSelected(func(r *gtk.ListBoxRow) {
		...
		parent.ActivateAction("win.open-channel", glib.NewVariantString(ch.id))
	})
	...
	gtkutil.OnFirstMap(v, func() {
		lastOpenState.Get(func(id string) {
...
// something else.
func (v *ChannelView) SelectChannel(chID string) {
...
func (v *ChannelView) Invalidate() {
	...
	// keep := make(map[string]bool, len(v.channels))
...
func (v *ChannelView) deleteCh(id string) {
	ch, ok := v.channels[id]
	if !ok {
		return
	}
