...
type View struct {
	*adw.ToolbarView
	...
	guildID  string
	selectID string // delegate to select later
}
...
func NewView(ctx context.Context, guildID string) *View {
	// state.MemberState.Subscribe(guildID)

	v := View{
		model:   newModelManager(guildID),
		guildID: guildID,
	}
...
// something else.
func (v *View) SelectChannel(selectID string) bool {
...
func (v *View) findChannelItem(id string) (uint, bool) {
