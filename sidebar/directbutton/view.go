...
type View struct {
	*gtk.Box
	...
	mentioned struct {
		IDs     []string
		Buttons map[string]*ChannelButton
	}
...
func NewView(ctx context.Context) *View {
	...
	v.mentioned.IDs = make([]string, 0, 4)
	v.mentioned.Buttons = make(map[string]*ChannelButton, 4)
...
func (v *View) Invalidate() {
	...
	// var unreads map[string]channelUnreadStatus
	// for i, dm := range dms {
	...
	// 	if unreads == nil {
	// 		unreads = make(map[string]channelUnreadStatus, 4)
	// 	}
...
func (v *View) update(unreads map[string]channelUnreadStatus) {
